...
let
  ...
  buildAssets =
    versionInfo: assetsIndex:
    runCommand "assets" { } ''
      ${concatStringsSep "\n" (
        builtins.attrValues (
          flip mapAttrs assetsIndex.objects (
            name: a:
            let
              asset = fetchurl {
                sha1 = a.hash;
        ...
        let
          lemma1 =
            ...
            if rule.action == "allow" then
              ...
              false;
        in
        if artifact ? rules then foldl' lemma1 false artifact.rules else true;
  ...
  buildFabricModules =
    ...
    let
      ...
      extraJavaLibraries = buildFabricLibraries (fabricProfile.libraries.client ++ loader.libraries);
    in
    [
      (buildBasicModule versionInfo assetsIndex)
      ({
